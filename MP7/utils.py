...
def hyperparameters():
    ...
    parser.add_argument('--epsilon_min', type=float, default=0, help='minimum exploration parameter')
    parser.add_argument('--epsilon_decay_factor', type=float, default=1e-4, help='exploration decay parameter')
    ...
    parser.add_argument('--gamma', type=float, default=0.90, help='discount reward factor. represents how confident a '
                                                                  'model should be able to predict future rewards')
    parser.add_argument('--lr', type=float, default=.0175, help='learning rate')'''
...
def rollout(env: gym.Env, policies: policies.QPolicy, episodes: int, temp: float, render: bool = False):
    ...
    for itrnum in range(episodes):
        ...
        while not done:
            ...
            #action = np.random.choice([0,1,2],1,p=pi)[0]
            action = np.random.choice([0,1],1,p=pi)[0]
